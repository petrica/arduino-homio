...
class DeviceTransportTest : public Test {
    ...
    void SetUp() {
        ...
        commandPool = new NiceMock<CommandPoolMock>();
...
TEST_F(DeviceTransportTest, WhenHandlingCommandAndCommandQueueEmptyReturnFalse) {
    Command command;

    ON_CALL(*commandPool, borrowCommandInstance)
        .WillByDefault(Return(nullptr));

    bool expected = underTest->handleCommand(&command);

    ASSERT_FALSE(expected);
}

TEST_F(DeviceTransportTest, WhenHandlingCommandAndReceivedConfirmCommandThenCommandInstanceShouldBeReturnedToPool) {
    Command instance;
...
TEST_F(DeviceTransportTest, WhenHandlingCommandAndReceivedDatapointDeliverCommandThenCommandInstanceShouldNotBeReturnedToPool) {
    Command instance;
...
TEST_F(DeviceTransportTest, WhenHandlingCommandAndReceivedDatapointDeliverCommandAndDeliverCommandPendingThenExistingCommandInstanceShouldBeReturnedToPool) {
    Command instance;
    ...
    EXPECT_CALL(*commandPool, returnCommandInstance(Eq(&existing)));
    underTest->setReceivedCommand(&existing);

    underTest->handleCommand(&command);
}

TEST_F(DeviceTransportTest, WhenDeviceTransportIsInitialisedThenReceivedCommandIsNull) {
    ASSERT_THAT(underTest->getReceivedCommand(), Eq(nullptr));
}

TEST_F(DeviceTransportTest, WhenNoCommandReceivedThenReadingDatapointShouldReturnFalse) {
    Datapoint datapoint;
